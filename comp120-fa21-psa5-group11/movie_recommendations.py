...
Date: 11/10/21
Author: Long Pham and Natalie Nguyen
Description: A movie recommendation system that predicts ratings for movies that users have not seen and
computes similarities between movies
"""
...
class Movie_Recommendations:
    ...
    def __init__(self, movie_filename, training_ratings_filename):
        ...
        self.ratings_file = open(training_ratings_filename, encoding = "utf-8")
        ...
        # iterate through the ratings file and if the temp_user_id doesn't match the user id in the current line of the file or is None,
        ...
        for line in self.rating_csv_reader:
            if self.temp_user_id is None:
                self.temp_user_id = int(line[0])
    ...
    def predict_rating(self, user_id, movie_id):
        ...
        # check if the movie_id and user_id is valid and in the database
        ...
        # iterate through the movie dictionary and check if the user has rated other movies and if so add up the similarities for those movies
        # and add up the products of the similarities and ratings for the movies the user has watched
        for other_movie in self.movie_dict:
            if other_movie == movie_id:
                continue

            if other_movie in self.user_dict[user_id]:
                sum = sum + self.movie_dict[movie_id].get_similarity(other_movie, self.movie_dict, self.user_dict) * self.user_dict[user_id][other_movie]
                sim_sum = sim_sum + self.movie_dict[movie_id].get_similarity(other_movie, self.movie_dict, self.user_dict)
        
        # if the sum of the similarities is 0, return a default rating
        # otherwise return the sum of the products of the similarities and ratings divided by the sum of the similarities
        if sim_sum == 0:
            return 2.5
    ...
    def predict_ratings(self, test_ratings_filename):
        ...
        self.f = open(test_ratings_filename, encoding = "utf-8")
        self.test_csv_reader = csv.reader(self.f, delimiter = ',', quotechar = '"')
        self.f.readline()
        
        rating_list = []
        
        for line in self.test_csv_reader:
            # append tuple to list for each line in the file
            rating_list.append((int(line[0]), self.movie_dict[int(line[1])].title, self.predict_rating(int(line[0]), int(line[1])), float(line[2])))
...
if __name__ == "__main__":
    ...
    movie_recs = Movie_Recommendations("dummy_movies.csv", "dummy_training_ratings.csv")

    # Predict ratings for user/movie combinations
    rating_predictions = movie_recs.predict_ratings("dummy_test_ratings.csv")
